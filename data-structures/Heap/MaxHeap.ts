...
class MaxHeap implements Heap {
  ...
  siftDown = (index: number) => {
    ...
    let biggestChildIndex = rightChildIndex >= this.size() ? leftChildIndex : (this.elements[leftChildIndex] > this.elements[rightChildIndex] ? leftChildIndex : rightChildIndex);
    if (this.elements[biggestChildIndex] > this.elements[index]) {
      // swap inplace: https://en.wikipedia.org/wiki/XOR_swap_algorithm
      this.elements[biggestChildIndex] ^= this.elements[index];
      this.elements[index] ^= this.elements[biggestChildIndex];
      this.elements[biggestChildIndex] ^= this.elements[index];
      return this.siftDown(biggestChildIndex);
    }
  }
