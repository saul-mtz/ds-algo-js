...
const testAdjacent = (graph) => test('adjacent', () => {
...
const testNeighbors = (graph) => test('neighbors', () => {
  ...
  expect(toArray(graph.neighbors(1))).toEqual(expect.arrayContaining([[ 2,  24 ], [3, 3], [4, 20]]));
  expect(toArray(graph.neighbors(2))).toEqual(expect.arrayContaining([[ 1,  24 ]]));
  expect(toArray(graph.neighbors(3))).toEqual(expect.arrayContaining([[ 1, 3], [4,  12 ]]));
  expect(toArray(graph.neighbors(4))).toEqual(expect.arrayContaining([[1, 20], [ 3,  12 ]]));
});
